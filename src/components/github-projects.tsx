...
async function fetchGitHubProjects(username: string) {
  const response = await fetch(
    `https://api.github.com/users/${username}/repos?type=all&sort=stars&per_page=100&direction=desc`,
  );
  if (!response.ok) {
...
const featuredProjects = {
  PhotoGIMP: {
    ...
    category: 'opensource',
    impact:
      'Patch open source que reorganiza o GIMP 2.10+ no layout do Photoshop — novos filtros em Python, tela inicial própria, configurações otimizadas. Mantido desde 2021, com usuários reais.',
  ...
  bitqueue: {
    ...
    category: 'backend',
    impact:
      'Sistema de mensageria confiável e escalável para desacoplar microsserviços e aplicações serverless',
  ...
  'techfinance-previsao': {
    ...
    category: 'ai',
    impact:
      'Modelo de Machine Learning para previsão de séries temporais financeiras usando Python/Prophet',
...
function ProjectCard({
  ...
  return (
    <Card
      className={`group relative overflow-hidden transition-all duration-500 h-full flex flex-col hover:-translate-y-1 ${
        isFeatured
    ...
    >
      <div
        className={`absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-vesper-orange/50 to-transparent ${isFeatured ? 'opacity-60' : 'opacity-0'} group-hover:opacity-100 transition-opacity`}
      />
      ...
      <CardHeader className="pb-4 space-y-4 z-10">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0 pt-1">
            <CardTitle
              className={`text-lg font-bold leading-tight transition-colors group-hover:text-vesper-orange ${
            ...
            {project.language && (
              <div className="flex items-center gap-1.5 mt-1.5">
                ...
                />
                <p className="text-[11px] text-foreground/55 font-mono">{project.language}</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="flex gap-2 pt-1">
            {featuredInfo.techStack.map((tech) => {
              ...
              return (
                <div
                  key={tech}
                  className="p-1.5 rounded bg-background/50 border border-vesper-orange/10"
                  title={tech}
                >
                  {loadTechIcons ? (
                    <LazyDevIcon
        ...
        {project.topics.length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            ...
            {project.topics.length > (isFeatured ? 5 : 3) && (
              <Badge
                variant="outline"
                className="text-[10px] border-vesper-orange/10 text-vesper-orange/65 bg-transparent"
              >
                +{project.topics.length - (isFeatured ? 5 : 3)}
              </Badge>
            )}
      ...
      <CardFooter
        className={`flex justify-between items-center text-sm border-t pt-4 z-10 ${
