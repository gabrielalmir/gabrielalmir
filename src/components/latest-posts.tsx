        ...
        <div className="grid gap-6 sm:gap-8 md:grid-cols-2 lg:grid-cols-3 w-full">
          {latestPosts.map((post) => {
            ...
            return (
              ...
              >
                <div
                  className={`absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent ${category.color === 'text-vesper-cyan' ? 'via-vesper-cyan/50' : category.color === 'text-vesper-red' ? 'via-vesper-red/50' : 'via-vesper-orange/50'} to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300`}
                />
                ...
                <div className="flex items-start justify-between mb-4 relative z-10 gap-3">
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    <CategoryIcon
                      className={`h-4 w-4 sm:h-4 sm:w-4 flex-shrink-0 ${category.color}`}
                    />
                    <span
                      className={`text-[10px] sm:text-xs uppercase tracking-[0.18em] font-semibold ${category.color}`}
                    >
                      {category.label}
                    </span>
                  </div>
