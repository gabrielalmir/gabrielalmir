            ...
            <div className="container mx-auto max-w-7xl">
              <div className="mb-8">
                ...
                <h1 className="section-heading mt-4">
                  <span className="text-vesper-orange/70">&lt;</span> DevLog{' '}
                  <span className="text-vesper-orange/70">/&gt;</span>
                </h1>
              ...
              {filteredPosts.length > 0 ? (
                <div className="grid gap-6 sm:gap-8 md:grid-cols-2 lg:grid-cols-3">
                  {filteredPosts.map((post, index) => {
                    const CategoryIcon =
                      categories[post.category as keyof typeof categories]?.icon || Code2;
                    const categoryInfo =
                      categories[post.category as keyof typeof categories] || categories.technical;
                    return (
                      <m.article
                        key={post.id}
                        initial={{ opacity: 0, y: 20 }}
                      ...
                      >
                        <div
                          className={`absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-${categoryInfo.color === 'text-vesper-cyan' ? 'vesper-cyan' : 'vesper-orange'}/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300`}
                        />
                        ...
                        <div className="flex items-start justify-between mb-4 relative z-10 gap-3">
                          <div className="flex items-center gap-2 flex-1 min-w-0">
                            <CategoryIcon
                              className={`h-4 w-4 flex-shrink-0 ${categoryInfo.color}`}
                            />
                            <span
                              className={`text-[10px] uppercase tracking-[0.18em] font-semibold ${categoryInfo.color}`}
                            >
                              {categoryInfo.label}
                            </span>
                          </div>
                ...
                >
                  ...
                  <div className="text-foreground/40 text-xs sm:text-sm font-mono overflow-x-auto inline-block">
                    <span className="text-vesper-orange/60">$</span> find ./posts -name "*
                    {searchTerm}*" | wc -l
                    <br />
