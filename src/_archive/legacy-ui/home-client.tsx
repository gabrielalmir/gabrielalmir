          ...
          <div className="space-y-8 md:space-y-10 w-full max-w-full overflow-hidden z-10">
            ...
            <div className="space-y-4 md:space-y-6 w-full max-w-full">
              ...
              <p className="text-base sm:text-lg md:text-xl text-foreground/65 max-w-xl leading-[1.65] break-words w-full mt-2">
                Integro <span className="text-vesper-orange/95 font-medium">sistemas ERP</span> e
                construo <span className="text-vesper-orange/95 font-medium">APIs em Node.js</span>{' '}
                para ambientes regulados — onde uma falha de sincronização é não-conformidade, não
                só um bug.
              </p>
            </div>
            ...
            <div className="flex items-center gap-1 pt-2 w-full max-w-full flex-wrap">
              <span className="text-[10px] uppercase tracking-[0.2em] text-foreground/60 font-mono mr-3">
                Conecte
              </span>
...
function SkillsSection() {
  return (
    <section
      id="habilidades"
      className="py-20 md:py-28 px-4 sm:px-6 lg:px-8 w-full max-w-full overflow-hidden relative"
    >
      ...
      <div className="container mx-auto max-w-7xl w-full relative z-10">
        <SectionHeader
          ...
          title={
            <>
              ...
              <span className="text-vesper-orange">Técnico</span>
            </>
          }
        ...
        <div className="space-y-16 w-full max-w-full">
          <DeferredRender
            fallback={
              <div className="h-96 w-full animate-pulse bg-vesper-orange/5 rounded-xl"></div>
            }
          >
            <Suspense
              fallback={
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap gap-2">
                {dataMessagingTech.map((tech) => (
                  <span
                    key={tech}
                    className="px-3 py-1.5 rounded-md border border-vesper-cyan/20 bg-vesper-cyan/5 text-vesper-cyan/90 text-sm hover:bg-vesper-cyan/10 transition-colors cursor-default"
                  >
                    {tech}
                  </span>
                ))}
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap gap-2">
                {cloudDevOpsTech.map((tech) => (
                  <span
                    key={tech}
                    className="px-3 py-1.5 rounded-md border border-foreground/10 bg-foreground/5 text-foreground/80 text-sm hover:bg-foreground/10 transition-colors cursor-default"
                  >
                    {tech}
                  </span>
                ))}
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap gap-2">
                {integrationsTech.map((tech) => (
                  <span
                    key={tech}
                    className="px-3 py-1.5 rounded-md border border-vesper-orange/20 bg-vesper-orange/5 text-vesper-orange/90 text-sm hover:bg-vesper-orange/10 transition-colors cursor-default"
                  >
                    {tech}
                  </span>
                ))}
...
function ProjectsSection({ githubProjects }: { githubProjects?: any[] }) {
  return (
    <section
      id="projetos"
      className="py-20 md:py-28 bg-gradient-to-b from-background via-background to-vesper-orange/[0.03] px-4 sm:px-6 lg:px-8 w-full max-w-full overflow-hidden"
    >
      <div className="container mx-auto max-w-6xl w-full">
        <SectionHeader
          ...
          title={
            <>
              ...
              <span className="text-vesper-orange">Destaque</span>
            </>
          }
        ...
        <DeferredRender
          ...
          fallback={
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              {Array.from({ length: 4 }, (_, index) => `home-skeleton-${index}`).map(
                (skeletonId) => (
                  <div
                    key={skeletonId}
                    className="terminal-window border border-vesper-orange/20 p-4 sm:p-6 animate-pulse"
                  >
        ...
        >
          <Suspense
            fallback={
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                {Array.from({ length: 4 }, (_, index) => `home-skeleton-suspense-${index}`).map(
                  (skeletonId) => (
                    <div
                      key={skeletonId}
                      className="terminal-window border border-vesper-orange/20 p-4 sm:p-6 animate-pulse"
        ...
        <div className="text-center mt-16">
          <a
            href="https://github.com/gabrielalmir?tab=repositories"
            target="_blank"
            rel="noopener noreferrer"
          >
          ...
          <div className="relative z-10 flex flex-col items-center text-center">
            ...
            <div className="flex flex-wrap gap-4 justify-center mb-12">
              <a href="mailto:gabr.almir@gmail.com" className="w-full sm:w-auto">
                <Button
                  size="lg"
                  className="group w-full sm:w-auto bg-vesper-orange hover:bg-vesper-orange/90 text-black font-bold shadow-lg shadow-vesper-orange/20 hover:shadow-xl hover:shadow-vesper-orange/40 transition-all duration-300 px-8 h-14 text-base"
                >
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          <div className="space-y-6">
            ...
            <p className="text-sm text-foreground/70 leading-relaxed max-w-xs">
              Desenvolvedor Backend Node.js/TypeScript, com foco em integrações ERP e sistemas em
              ambientes regulados.
            </p>
          </div>

          <div className="space-y-6">
            <h4 className="text-sm font-bold text-foreground uppercase tracking-widest border-b border-vesper-orange/10 pb-2 w-fit">
              Navegação
            </h4>
        ...
        <div className="border-t border-vesper-orange/10 pt-8">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2 text-foreground/60 text-xs font-mono">
              <a href="/sitemap-index.xml" className="hover:text-vesper-orange transition-colors">
                Sitemap
              </a>
              <a
...
export function HomeClient({ latestPosts, githubProjects }: HomeClientProps) {
  ...
  return (
    <LazyMotion features={domAnimation}>
      <div
        data-home-loaded
        className="min-h-screen bg-background text-foreground selection:bg-primary selection:text-black overflow-x-hidden"
      >
        <m.div
        ...
        <main id="main" className="w-full overflow-x-hidden max-w-full">
          ...
          <section
            id="sobre"
            className="py-20 md:py-28 px-4 sm:px-6 lg:px-8 w-full max-w-full overflow-hidden"
          >
            <div className="container mx-auto max-w-7xl w-full">
              <ModernResumeSection />
            </div>
